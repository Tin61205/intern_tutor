...
class Settings(BaseSettings):
    ...
    # --- Auth Settings ---
    SECRET_KEY: str = Field(..., min_length=32, description="Secret key phải dài ít nhất 32 ký tự")
    ALGORITHM: str = "HS256"
    ACCESS_TOKEN_EXPIRE_MINUTES: int = Field(default=30, gt=0, description="Thời gian sống của Token phải lớn hơn 0")

    # --- Redis Settings ---
    REDIS_HOST: str = Field(default="localhost", min_length=1)
    REDIS_PORT: int = Field(default=6379, ge=1, le=65535, description="Port hợp lệ từ 1 đến 65535")
